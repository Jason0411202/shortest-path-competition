...
static vector<uint16_t> L16;         // first LBL lanes of DIN, 0xFFFF = unknown (>= 65535)
static int L16ON = 1;
...
static D query(int32_t s, int32_t t) {
    ...
    const D* rowt = HUBS ? &DIN[(size_t)t * K] : nullptr;
    D minin = INF;                  // min over hubs h != t of d(h, t)
    if (HUBS) for (int k = 0; k < K; ++k) { D x = hubs[k] != t ? rowt[k] : INF; minin = x < minin ? x : minin; }
    for (;;) {
        if (hF.empty()) break;
        D kf = hF.topkey();
        bool meet = false;
        ...
        if (fwd) {
            ...
            if (HUBS) {
                ...
                if (mu != INF) {                           // landmark bound on d(u, t)
                    int32_t lb = 0;
                    if (L16ON) {
                        const uint16_t* ru = &L16[(size_t)u * LBL];
                        for (int k = 0; k < LBL; ++k) {
                        ...
                        for (int k = 0; k < LBL; ++k) {
                            int32_t v = ru[k] != INF ? (int32_t)(rowt[k] - ru[k]) : 0;
                            lb = v > lb ? v : lb;
                        }
                    }
            ...
            for (uint32_t j = 0; j < deg; ++j) {
                ...
                if (nd < S.d[0]) {
                    ...
                    if (HUBS && PFROW) {
                        const char* q = L16ON ? (const char*)&L16[(size_t)x * LBL] : (const char*)&DIN[(size_t)x * K];
                        const int bytes = L16ON ? LBL * 2 : LBL * 4;
                        for (int b = 0; b < bytes; b += 64) __builtin_prefetch(q + b);
                    }
                    hF.push(nd, sx);
                    if (TB.full()) rehash();
                }
...
void solve() {
    ...
    if (K > 0) {
        ...
        if (const char* e = std::getenv("SF_L16")) L16ON = std::atoi(e);
        if (L16ON && LBL > 0) {
            L16.resize((size_t)V * LBL);
            for (int32_t v = 0; v < V; ++v) {
                const D* r = &DIN[(size_t)v * K];
                uint16_t* o = &L16[(size_t)v * LBL];
                for (int k = 0; k < LBL; ++k) o[k] = r[k] < 0xFFFF ? (uint16_t)r[k] : (uint16_t)0xFFFF;
            }
        } else L16ON = 0;
        tlog("hub table");
    }
