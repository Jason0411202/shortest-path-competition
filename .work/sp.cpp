...
static int USE_PLL = 0, USE_BIDIR = 0;
static int STALL = 1;
static int SHUF = 0;
...
static void read_graph(const char* path) {
    ...
    for (int32_t i = 0; i < E; ++i) {
        ...
        W w = (W)sc.uint_();
        if (u == v) continue;
        if (SHUF) { u = (int32_t)(((uint64_t)u * 2654435761ull) % (uint64_t)V); v = (int32_t)(((uint64_t)v * 2654435761ull) % (uint64_t)V); }
...
static int USE_HL = 0, HL_PRUNE = 1;
...
static uint64_t g_qhist[21], g_qwork[21];
...
static int64_t ch_query(int32_t s, int32_t t) {
    ...
    if (g_debug) {
        size_t tsz = touchedF.size() + touchedB.size();
        int b = 0; while ((1u << b) < tsz && b < 20) ++b;
        g_qhist[b] += 1; g_qwork[b] += tsz;
    }
...
int main(int argc, char** argv) {
    ...
    run_queries(argv[2], argv[3]);
    tlog("queries");
    if (g_debug) for (int b = 0; b <= 20; ++b) if (g_qhist[b]) std::fprintf(stderr, "touched<=2^%d: %llu queries, %llu touched\n", b, (unsigned long long)g_qhist[b], (unsigned long long)g_qwork[b]);
    return 0;
}
