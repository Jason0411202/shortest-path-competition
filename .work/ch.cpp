...
static void read_graph(const char* path) {
    ...
    for (int32_t i = 0; i < E; ++i) {
        ...
        W w = (W)sc.uint_();
        if (u == v) continue;
        if (USE_PLL) { raw_u.push_back(u); raw_v.push_back(v); raw_w.push_back(w); continue; }
...
static void run_queries(const char* qpath, const char* opath) {
    ...
    if (!USE_PLL) { dF.assign((size_t)V, INF); dB.assign((size_t)V, INF); }
    ...
    for (int32_t i = 0; i < Q; ++i) {
        ...
        int64_t d = USE_PLL ? pll_query(s, t) : USE_HL ? hl_query(rank_of[s], rank_of[t]) : ch_query(rank_of[s], rank_of[t]);
        if (d < 0) { *o++ = '-'; *o++ = '1'; }
